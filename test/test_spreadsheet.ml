...
let test1 () =
  ...
  let three_cells : (int cell * int cell * int cell) exp =
    ...
    cell
      ( get a >>= fun aValue ->
        get b >>= fun bValue -> return (aValue + bValue) )
    >>= fun c -> return (a, b, c)
  in
  ...
  (* a = b*b
         ...
         So c = a + b = 4*4 + 4 = 20
       *)
  set a (get b >>= fun bValue -> return (bValue * bValue));

  set b (return 4);
  let c20 = run (get c) in

  Alcotest.(check (triple int int int)) "equal" (3, 102, 20) (c3, c102, c20)
...
let test2 () =
  ...
  let three_cells : (string cell * int cell * int cell) exp =
    ...
    cell
      ( get a >>= fun aValue ->
        get b >>= fun bValue -> return (String.length aValue + bValue) )
    >>= fun c -> return (a, b, c)
  in
  ...
  Alcotest.(check (pair int int)) "equal" (7, 5) (c7, c5)
...
let () =
  ...
  run "spreadsheet"
    [
      ( "tests",
        [
          test_case "propagate" `Quick test1;
          test_case "propagate when cells are different types" `Quick test2;
        ] );
    ]
